...
public class UserQuizController {
    ...
    @Operation(
            summary = "Save draft attempt answer",
            description = "Saves or updates one selected option for an in-progress attempt so the attempt can be resumed.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    public ResponseEntity<ApiResponse<Void>> saveAttemptAnswer(
            @PathVariable @Positive Long attemptId,
            @Valid @RequestBody SaveAttemptAnswerRequest request,
            @AuthenticationPrincipal CustomUserDetails currentUser
    ) {
        userQuizService.saveAttemptAnswer(attemptId, currentUser.getUser().getId(), request);
